...
export function setUnblockFunction(fn: (user: string) => void) {
  unblockFn = fn
}

export const cp = {
  init: function (page: number) {
    const $ = (window as any).jQuery

    // Create the settings button
    $('<div id="ext_settings_button"><img src="' + browser.runtime.getURL('/images/settings/icon.png') + '" alt="SG tuning beállítások" title="SG tuning beállítások"></div>').appendTo('body')
    ...
    html += '<div id="ext_settings_wrapper">'
      ...
      html += '<div class="settings_page">'
        ...
        html += '</div>'
        html += '<div>'
          html += '<h3>Megnyitott oldalak kiemelése</h3>'
          html += '<p>A már megnyitott topikok, kategóriák (egy másik böngésző fülön) legyenek kiemelve a főoldalon, topikokban.</p>'
          html += '<div class="button" id="sgTabs"></div>'
        html += '</div>'
      html += '</div>'
    ...
    const ext_header = $('#ext_settings_header')
    const settings_button = $('.settings_page .button')
    ...
    settings_button.css({ 'background-image': 'url(' + browser.runtime.getURL('/images/settings/button.png') + ')' })

    // Get the requested page number
    const sPage = typeof page === 'undefined' ? 0 : page
    ...
    $('#reset_blocks_config').click(function () {
      context.port!.postMessage({name: 'resetBlocksConfig'})
    })
  ...
  show: function () {
    ...
    const ext_h_overlay = $('#ext_settings_hide_overlay')
    const ext_s_wrapper = $('#ext_settings_wrapper')
    ...
    const paneWidth = ext_s_wrapper.width()
    const paneHeight = ext_s_wrapper.height()
    const leftProp = viewWidth / 2 - paneWidth / 2
  ...
  tab: function (index: number) {
    const $ = (window as any).jQuery

    const ext_s_wrapper = $('#ext_settings_wrapper')
    const settings_page = $('.settings_page')
    const ext_settings_header = $('#ext_settings_header')
    ...
    const newPaneHeight = ext_settings_header.height() + settings_page.eq(index).outerHeight()
  ...
  button: function (ele: HTMLElement) {
    const $ = (window as any).jQuery
...
export const blocklist_cp = {
  ...
  list: function () {
    ...
    if (!context.dataStore['blocklisted']) {
      return false
    }

    const blocklist = $('#ext_block-list')
    // Everything is OK, remove the default message
    blocklist.html('')

    // Fetch the user list into an array
    const users = context.dataStore['blocklisted'].split(',').sort()
  ...
  remove: function (el: HTMLElement) {
    ...
    // Get username
    const user = $(el).prev().html()

    // Remove user from the list
    $(el).closest('li').remove()

    // Remove user from preferences
    context.port!.postMessage({ name: 'removeUserFromBlocklist', message: user })
...
export const settings = {
  ...
  save: function (ele: HTMLElement) {
    const $ = (window as any).jQuery

    if ($(ele).hasClass('on') || $(ele).prop('checked') === true || $(ele).is(':checked')) {
      // Save new settings ...
      context.port!.postMessage({ name: 'setSetting', key: $(ele).attr('id'), val: true })

      // Set new value to dataStore var
      context.dataStore[$(ele).attr('id')] = true
      ...
      // Save new settings ...
      context.port!.postMessage({ name: 'setSetting', key: $(ele).attr('id'), val: false })

      // Set new value to dataStore var
      context.dataStore[$(ele).attr('id')] = false
  ...
  select: function (ele: HTMLElement) {
    ...
    const val = $(ele).find('option:selected').val()

    // Update in dataStore
    context.dataStore[$(ele).attr('id')] = val

    // Update in localStorage
    context.port!.postMessage({ name: 'setSetting', key: $(ele).attr('id'), val: val })
  },
...
const profiles_cp = {
  init: function () {
    ...
    $('.settings_page a.new_profile').on('click', function (e: Event) {
      e.preventDefault()
    ...
    // Save the settings
    $('.settings_page .profile_save').on('click', function (e: Event) {
  ...
  rebuildProfiles: function () {
    ...
    if (!context.dataStore['profilesList']) {
      return false
    }
    ...
    for (let c = 0; c < profiles.length; c++) {
      ...
      const clone = $('.settings_page .profiles li.sample').clone()

      // Get the target element
      const target = $('.settings_page .profiles')

      // Append the new group
      const content = $(clone).appendTo(target).removeClass('sample')
  ...
  removeGroup: function (ele: HTMLElement) {
    const $ = (window as any).jQuery
  ...
  changeColor: function (ele: HTMLElement) {
    const $ = (window as any).jQuery

    // Get selected color
    const color = $(ele).find('span').html().split(',')
  ...
  save: function () {
    ...
    context.port!.postMessage({ name: 'setSetting', key: 'profilesList', val: JSON.stringify(data) })

    // Save new settings in dataStore
    context.dataStore['profilesList'] = JSON.stringify(data)
