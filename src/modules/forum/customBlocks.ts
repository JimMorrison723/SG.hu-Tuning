...
customBlocks.activate = () => {
  ...
  if (!context.dataStore['blocksConfig'] || context.dataStore['blocksConfig'] === '') {
  ...
  // Set overlays
  if (!context.dataStore['hideBlocksButtons']) {
...
customBlocks.setIDs = () => {
  ...
  $('#sidebar-forum > section, #forum-wrap > div, #forum-wrap > section').each(function () {
    ...
    if ($(this).attr('id') === undefined) $(this).attr('id', 'block-' + counter)
...
customBlocks.buildConfig = () => {
  ...
  if (context.port) {
    context.port.postMessage({ name: 'setBlocksConfig', message: JSON.stringify(config) })
  }

  // Update in dataStore let
  context.dataStore['blocksConfig'] = JSON.stringify(config)
}
...
customBlocks.reindexOrderConfig = () => {
  ...
  // Store in localStorage
  if (context.port) {
    context.port.postMessage({ name: 'setBlocksConfig', message: JSON.stringify(_config) })
  }
}
...
customBlocks.setOverlay = () => {
  $('.ext_block').each(function () {
    const item = $('<p class="ext_blocks_buttons"></p>').prependTo(this)

    // Content hide
    $('<img src="' + browser.runtime.getURL('/images/blocks/minimize.png') + '" class="ext_block_button_right">').prependTo(item).click(function (e) {
      e.preventDefault()
      customBlocks.contentHide($(this).closest('section, div').attr('id') || '', true)
    })

    // Hide
    $('<img src="' + browser.runtime.getURL('/images/blocks/close.png') + '" class="ext_block_button_right">').prependTo(item).click(function (e) {
      e.preventDefault()
      customBlocks.hide($(this).closest('section, div').attr('id') || '', true)
    })

    // Down
    $('<img src="' + browser.runtime.getURL('/images/blocks/down.png') + '" class="ext_block_button_left">').prependTo(item).click(function (e) {
      e.preventDefault()
      customBlocks.down($(this).closest('section, div').attr('id') || '', true)
    })

    // Up
    $('<img src="' + browser.runtime.getURL('/images/blocks/up.png') + '" class="ext_block_button_left">').prependTo(item).click(function (e) {
      e.preventDefault()
...
customBlocks.hide = (id: string, clicked: boolean) => {
  if (clicked === true) {
    // Change the config
...
customBlocks.contentHide = (id: string, clicked: boolean) => {
  const c_id = $('#' + id)

  const element_hide = customBlocks.getChildrenToHide(c_id)
...
customBlocks.getChildrenToHide = (parent: JQuery) => {
...
customBlocks.up = (id: string) => {
  const c_id = $('#' + id)

  // Get index val
  const index = c_id.index('.ext_block')
  ...
  if (c_id.closest('#ext_left_sidebar').length > 0) {
    ...
    const first = $('#ext_left_sidebar').find('.ext_block').length
    if (index === first) {
      return false
    }
  }
...
customBlocks.down = (id: string) => {
  ...
  // Current position
  if (c_id.closest('#ext_left_sidebar').length > 0) {
    const last = $('#ext_left_sidebar').find('.ext_block').length - 1
